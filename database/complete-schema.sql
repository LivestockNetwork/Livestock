...
-- Complete database schema for livestock emergency management
...
CREATE TABLE IF NOT EXISTS public.livestock (
    ...
    animal_type TEXT NOT NULL, -- cattle, sheep, horses, pigs, etc.
    breed TEXT,
    count INTEGER NOT NULL DEFAULT 1,
    age_group TEXT, -- calves, yearlings, adults, etc.
    location_paddock TEXT,
    special_needs TEXT,
    identification_method TEXT, -- ear tags, brands, etc.
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS public.emergency_plans (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
    plan_type TEXT NOT NULL, -- bushfire, flood, drought, disease
...
CREATE TABLE IF NOT EXISTS public.community_groups (
    ...
    description TEXT,
    location TEXT,
    group_type TEXT, -- local, regional, breed-specific, etc.
...
CREATE TABLE IF NOT EXISTS public.community_posts (
    ...
    post_type TEXT DEFAULT 'general', -- general, emergency, resource, help
...
CREATE TABLE IF NOT EXISTS public.emergency_alerts (
    ...
    alert_type TEXT NOT NULL, -- fire, flood, disease, weather
    severity TEXT NOT NULL, -- low, medium, high, critical
    affected_areas TEXT[], -- array of location names
...
CREATE TABLE IF NOT EXISTS public.resource_sharing (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
    resource_type TEXT NOT NULL, -- equipment, feed, transport, labor
    resource_name TEXT NOT NULL,
    description TEXT,
    availability_type TEXT NOT NULL, -- offer, request
    location TEXT,
    contact_method TEXT,
...
CREATE TABLE IF NOT EXISTS public.help_requests (
    ...
    help_type TEXT NOT NULL, -- evacuation, feed, veterinary, transport
...
CREATE POLICY "Group admins can update their groups" ON public.community_groups
...
CREATE POLICY "Users can create posts in their groups" ON public.community_posts
...
CREATE POLICY "Anyone can view unresolved help requests" ON public.help_requests
    FOR SELECT USING (is_resolved = false);
...
CREATE INDEX IF NOT EXISTS idx_emergency_alerts_active ON public.emergency_alerts(is_active);
CREATE INDEX IF NOT EXISTS idx_resource_sharing_available ON public.resource_sharing(is_available);
CREATE INDEX IF NOT EXISTS idx_help_requests_resolved ON public.help_requests(is_resolved);

-- Insert some sample data for testing
INSERT INTO public.community_groups (name, description, location, group_type, is_public) VALUES
