...
DROP POLICY IF EXISTS "Users can manage their own livestock" ON public.livestock;
DROP POLICY IF EXISTS "Users can manage their own emergency plans" ON public.emergency_plans;
...
DROP POLICY IF EXISTS "Anyone can view active emergency alerts" ON public.emergency_alerts;
DROP POLICY IF EXISTS "Anyone can view available resources" ON public.resource_sharing;
DROP POLICY IF EXISTS "Users can manage their own resources" ON public.resource_sharing;
DROP POLICY IF EXISTS "Anyone can view unresolved help requests" ON public.help_requests;
DROP POLICY IF EXISTS "Users can manage their own help requests" ON public.help_requests;
...
-- Create RLS policies (after dropping existing ones)
...
-- Insert sample community groups (only if they don't exist)
INSERT INTO public.community_groups (name, description, location, group_type, public) 
SELECT 'Manning Valley Farmers', 'Local farming community in the Manning Valley region', 'Manning Valley, NSW', 'regional', true
WHERE NOT EXISTS (SELECT 1 FROM public.community_groups WHERE name = 'Manning Valley Farmers');

INSERT INTO public.community_groups (name, description, location, group_type, public) 
SELECT 'Cattle Breeders Network', 'Network for cattle breeders across Australia', 'Australia-wide', 'breed-specific', true
WHERE NOT EXISTS (SELECT 1 FROM public.community_groups WHERE name = 'Cattle Breeders Network');

INSERT INTO public.community_groups (name, description, location, group_type, public) 
SELECT 'Emergency Response Team', 'Rapid response team for livestock emergencies', 'Regional', 'emergency', true
WHERE NOT EXISTS (SELECT 1 FROM public.community_groups WHERE name = 'Emergency Response Team');
