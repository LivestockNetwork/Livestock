-- LIVESTOCK EMERGENCY SYSTEM - COMPLETE DATABASE SECURITY SETUP
...
-- Enable RLS on emergency_plans table
...
USING (user_id = auth.uid() OR ((auth.jwt() ->> 'app_metadata')::jsonb ? 'is_admin'));
...
USING (id = auth.uid() OR ((auth.jwt() ->> 'app_metadata')::jsonb ? 'is_admin'));
